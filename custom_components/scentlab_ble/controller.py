...
def _device_timestamp() -> int:
    """Return the local-wall-clock timestamp generated by the Android app."""
    now = dt_util.now()
    utc_offset = now.utcoffset()
    return int(time.time()) + (
        int(utc_offset.total_seconds()) if utc_offset is not None else 0
    )
...
class ScentLabBleController:
    ...
    async def async_sync_time(self) -> None:
        """Synchronise the diffuser clock using ScentLab's local-time encoding."""
        timestamp = _device_timestamp()
        async with self._async_operation(), self._async_session() as session:
            await session.async_write(
                _build_frame(0x06, timestamp.to_bytes(4, "little")),
